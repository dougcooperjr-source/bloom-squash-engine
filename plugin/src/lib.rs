...
pub struct SpectralCompressor {
    ...
    /// between them without replanning or allocations.
    plan_for_order: [Plan; MAX_WINDOW_ORDER - MIN_WINDOW_ORDER + 1],
    ...
    analyzer_output_data: Arc<AtomicRefCell<triple_buffer::Output<AnalyzerData>>>,
    /// The exact frozen compressor snapshot used to render IR exports from the editor.
    frozen_ir_output_data: Arc<AtomicRefCell<triple_buffer::Output<FrozenIrData>>>,

    /// When true the plugin outputs the delta signal (input - processed) so the user can hear
    /// what the compressor is adding or removing.
...
impl Default for SpectralCompressor {
    fn default() -> Self {
        ...
        let (frozen_ir_input_data, frozen_ir_output_data) = TripleBuffer::default().split();
        ...
        SpectralCompressor {
            ...
            analyzer_output_data: Arc::new(AtomicRefCell::new(analyzer_output_data)),
            frozen_ir_output_data: Arc::new(AtomicRefCell::new(frozen_ir_output_data)),

            delta_active: Arc::new(AtomicBool::new(false)),
        }
    }
}
    ...
    fn default() -> Self {
        GlobalParams {
            ...
            output_gain: FloatParam::new(
                ...
                util::db_to_gain(0.0),
                FloatRange::Skewed {
                    min: util::db_to_gain(OUTPUT_GAIN_MIN_DB),
                    max: util::db_to_gain(OUTPUT_GAIN_MAX_DB),
                    factor: FloatRange::gain_skew_factor(OUTPUT_GAIN_MIN_DB, OUTPUT_GAIN_MAX_DB),
                },
            )
            .with_unit(" dB")
...
impl Plugin for SpectralCompressor {
    ...
    fn editor(&mut self, _async_executor: AsyncExecutor<Self>) -> Option<Box<dyn Editor>> {
        editor::create(
            ...
            editor::Data {
                ...
                mode_options: ThresholdMode::variants()
                    .iter()
    ...
    fn initialize(
        ...
        if self.stft.num_channels() != num_output_channels {
            self.stft = util::StftHelper::new(num_output_channels, MAX_WINDOW_SIZE, 0);
        }
    ...
    fn process(
        ...
        let synthesis_gain = gain_compensation.sqrt();
        let final_output_gain = self.params.global.output_gain.value();

        if self.match_runtime.begin_running_if_requested() {
            self.delta_active.store(false, Ordering::Relaxed);
            self.match_meter.start(self.buffer_config.sample_rate);
        }

        let match_active = self.match_meter.is_active();
        let match_frames = self.match_meter.frames_for_block(buffer.samples());
        if match_frames > 0 {
            self.match_meter.measure_input(buffer, match_frames);
        }

        // This is mixed in later with latency compensation applied
        self.dry_wet_mixer.write_dry(buffer);

        let delta_active = self.delta_active.load(Ordering::Relaxed) && !match_active;
        let bypass_active = self.params.global.bypass.value();
        let process_active = delta_active || match_active || !bypass_active;

        if process_active {
            // These plans have already been made so we can switch between
            // versions without reallocating.
            let fft_plan = &mut self.plan_for_order
                [self.params.global.window_size_order.value() as usize - MIN_WINDOW_ORDER];
            let num_bins = self.complex_fft_buffer.len();
            // The Hann window function spreads the DC signal out slightly, so we'll clear all
            // 0-20 Hz bins for this. With small window sizes you probably don't want this as it
            // would result in a significant low-pass filter. When it's disabled, the DC bin will
            // also be compressed.
            let first_non_dc_bin_idx = ((20.0
...
/// Shared immutable settings for [`process_stft_main()`].
struct StftMainConfig<'a> {
    fft_plan: &'a Plan,
    window_function: &'a [f32],
    params: &'a SpectralCompressorParams,
    input_gain: f32,
    synthesis_gain: f32,
    overlap_times: usize,
...
fn process_stft_main(
    ...
    for (sample, window_sample) in real_fft_buffer.iter_mut().zip(config.window_function) {
        *sample *= window_sample * config.input_gain;
    }
    ...
    config
        ...
        .expect("real-time forward FFT failed; buffer sizes are expected to match");
    ...
    compressor_bank.process(
        ...
        channel_idx,
        config.params,
        config.overlap_times,
        config.first_non_dc_bin_idx,
    );

    compressor_bank.publish_analyzer_if_needed();
    ...
    config
        ...
        .expect("real-time inverse FFT failed; buffer sizes are expected to match");
...
fn apply_output_gain(buffer: &mut Buffer, output_gain: f32) {
    ...
    for channel in buffer.as_slice() {
        channel.iter_mut().for_each(|sample| *sample *= output_gain);
    }
}
...
mod tests {
    ...
    #[test]
    fn output_gain_scales_channels() {
        let mut left = vec![1.0, -2.0];
...

        {
            ...
            unsafe {
                buffer.set_slices(2, |output_slices| {
                    *output_slices = vec![left.as_mut_slice(), right.as_mut_slice()];
                });
            }

            apply_output_gain(&mut buffer, 2.0);
        }
